...
//		// no SD card
                if (useFAT == 1) WaitPrompt("No SD Card found in WKF Slot !!");
		wkfInitialized = 0;
	}
	else {
		...
		if((wkfBuffer[0x1FF] != 0xAA)) {
			// No FAT!
	                if (useFAT == 1) WaitPrompt("No FAT Formatted WKF SD card found!!");
			wkfInitialized = 0;
		} 
		else {
			wkfInitialized = 1;
//	                if (useFAT == 1) ShowAction("Searching WKF SD Card");
		}
	}
}
